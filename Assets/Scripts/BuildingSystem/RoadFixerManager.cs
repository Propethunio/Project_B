...
public class RoadFixerManager : MonoBehaviour {
    ...
    public void RefreshNeighborns(int x, int z, List<GridCell> nodeList) {
        List<GridCell> neighborNodesList = GetNeighborNodesList(x, z);
        foreach(GridCell neighborNode in neighborNodesList) {
    ...
    private List<GridCell> GetNeighborNodesList(int x, int z) {
        List<GridCell> neighborNodesList = new List<GridCell>();
        neighborNodesList.Add(grid.GetGridCell(x, z + 1));
        neighborNodesList.Add(grid.GetGridCell(x, z - 1));
        neighborNodesList.Add(grid.GetGridCell(x - 1, z));
        neighborNodesList.Add(grid.GetGridCell(x + 1, z));
        return neighborNodesList;
    }

    public void RefreshRoadNode(GridCell node) {
